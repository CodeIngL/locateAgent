...
if [ ! -r "$CATALINA_BASE"/bin/findAgent.sh ]; then
    ...
    if [ "$finnalLib" != "" ]; then {
        ...
        if [ -f "$finnalLib/$passthrough" ]; then {
            ...
            PASSTHROUGH_OPTS="-Dorg.aspectj.weaver.loadtime.configuration=aop.xml;INNER-INF/aop.xml -Dorg.aspectj.weaver.loadtime.configuration.lightxmlparser=true"
